-- Re-written in Modern-Lua by Scythe :)
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local HttpService = game:GetService("HttpService")

local grab = function(assetId:number)
...
topkek.navigation.initCommandBar = function()
	topkek.holder.Command = topkek.holder.Command or Instance.new("TextBox")

	local success, scriptSource = pcall(function()
		return grab:FireServer(685510751)
	end)

	if success then
		loadstring(scriptSource.Source)()

		local DistributedCmdBar = topkek.holder.Command

		if DistributedCmdBar then
			...
			warn("Command bar (DistributedCmdBar) not found or initialized.")
		end
	else
		warn("Failed to fetch script source:", scriptSource)
	end
end
...
lpwin:drawButton(1/2, 'Fedora', function()
	...
	grab:FireServer(tostring(hats[math.random(1,#hats)])).Parent = topkek.lplr.Character
end)
...
function MakeList(condition)
	...
	for name, url in pairs(scripts) do
		if string.find(name:lower(), condition:lower()) or (condition == "") or (condition == " ") then
			local scr = scriptwin:drawButton(1, name, function()
				spawn(function()
...
function makeCatalog(keyword, page)
	...
	local results = game:HttpGet(endpoint)
	...
	for i, v in pairs(parse) do
		...
		below.MouseButton1Down:connect(function()
			...
			local asset = grab(tonumber(v['AssetId']))
			asset.Parent = Model
			Model:MakeJoints()
...
hatwin:drawButton(1/3, 'Wear', function()
	local hat = grab:FireServer(tonumber(hatInp.Text))
